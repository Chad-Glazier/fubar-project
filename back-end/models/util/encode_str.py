comma_replacement = "%2C" # Same as the URI encoding replacement.
...
def encode_str(string: str) -> str:
	...
	form.
	"""
	return string.replace(",", comma_replacement)

def decode_str(encoded_string: str) -> str:
	"""
	Take a string that was originally encoded by `encode_str` and restore it
	to its original content (i.e., including commas).
	"""
	return encoded_string.replace(comma_replacement, ",")
